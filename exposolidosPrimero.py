...
if response.status_code == 200:
    ...
    elementos_div = soup.find_all('div', class_='tab-content')
    #for i in range(len(elementos_div[0].contents)):
    for elemento_div in elementos_div[0].contents:
        print(elementos_div)
    # Iterar sobre los resultados
    for indice, resultado in enumerate(lista_resultados):        
        if indice >= 1:
            break        
        a_div = elemento_li.find('a', class_='tab-link')
        # Construir la URL completa
        url_completa = f"{base_url}/{resultado['href']}"
        ...
        if response.status_code == 200:
            ...
            elemento_div = soup.find_all('div', class_='tab-pane fade fusion-clearfix in active') 
            elementos_empresas = elemento_div[0].find_all('div', class_="expositores")
            ...
            print(f"No se pudo obtener el contenido de {url_completa}. Código de estado: {response.status_code}")
